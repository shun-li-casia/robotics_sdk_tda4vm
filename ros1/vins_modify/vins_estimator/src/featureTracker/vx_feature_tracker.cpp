...
FeatureTracker::trackImage(double _cur_time, const vx_image &_img,
                           const vx_image&_img1) {
  TicToc t_r;
  cur_time = _cur_time;
  cur_img = _img;

  vxQueryImage(cur_img, VX_IMAGE_HEIGHT, &row, sizeof(int));
  vxQueryImage(cur_img, VX_IMAGE_WIDTH, &col, sizeof(int));
  vx_image rightImg = _img1;
  cur_pts.clear();
  ...
  if (prev_pts.size() > 0) {
    ...
    if (hasPrediction) {
      cur_pts = predict_pts;
      vx_pyramid prev_pyramid =  vxCreatePyramid(vx_context_, 8, 1.2, 1920, 1536, VX_DF_IMAGE_U8);
      vx_pyramid cur_pyramid =  vxCreatePyramid(vx_context_, 8, 1.2, 1920, 1536, VX_DF_IMAGE_U8);
      vxuOpticalFlowPyrLK(vx_context_, prev_pyramid, cur_pyramid, );
